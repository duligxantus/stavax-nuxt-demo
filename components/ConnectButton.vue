...
import {TgBotScreen}                                          from '@stavaxio/account-sdk';
...
const $stavax = useStavaxAccount(false);

async function connectWallet() {
  const session = await $stavax.connect();
  if (!session) {
    console.error('cannot create stavax session');
    return;
  }

  $stavax.openTgBotWithSession(session);
}
...
<template>
  <div>
    ...
    <div v-if="isConnected" class="space-y-2">
      <div class="flex gap-2">
        <Button class="w-full" variant="outline" @click="$stavax.openTgBotScreen(TgBotScreen.deposit)">Deposit
        </Button>
        <Button class="w-full" variant="outline" @click="$stavax.openTgBotScreen(TgBotScreen.withdraw)">Withdraw
        </Button>
      </div>
      <div class="flex gap-2">
        <Button class="w-full" variant="outline" @click="$stavax.openTgBot()">Open Stavax Account</Button>
      </div>
      <Button
        class="w-full"
        variant="destructive"
